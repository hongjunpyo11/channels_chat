...
class ChatConsumer(WebsocketConsumer):
    ...
    def receive(self, text_data):
        ...
        if message_type == "draw":  # 그리는 중인 경우
            ...
            id_ = redis_conn.incr("line_id")
            save_coordinates_to_redis(id_, message_type, pointer)
...
def save_coordinates_to_redis(id, message_type, pointer):
    """
    id : 고유 아이디
    message_type : 메시지 타입
    pointer : 포인터 리스트 [[(x1, y1), (x2, y2), ...], [(x1, y1), (x2, y2), ...], ... ]
    """
    # JSON 형식으로 데이터를 묶습니다.
    data = {'id': id, 'message_type': message_type, 'pointer': pointer}
    json_data = json.dumps(data)
    ...
    # Redis에 데이터를 저장합니다.
    redis_conn.set(id, json_data)

def get_all_coordinates_from_redis():
    """
    Redis에 저장된 모든 데이터를 가져와서 반환합니다.
    """
    # Redis 연결을 가져옵니다.
    redis_conn = get_redis_connection()

    # Redis에 저장된 모든 데이터의 키를 가져옵니다.
    keys = redis_conn.keys()

    # 키에 해당하는 데이터를 JSON 형식으로 변환합니다.
    data_list = []
    for key in keys:
